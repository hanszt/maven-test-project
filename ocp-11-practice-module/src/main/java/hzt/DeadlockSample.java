...
public class DeadlockSample {
    ...
    //First thread acquires the lock of sb1. It appends X to sb1. Just after that,
    // the OS stops this thread and starts the second thread. (Note that first thread still has the lock of sb1).
    // Second thread acquires the lock of sb2 and appends Y to sb2. Now it tries to acquire the lock for sb1.
    // But sb1's lock is already acquired by the first thread so the second thread has to wait.
    // Now, the OS starts the first thread.

    // It tries to acquire the lock of sb2 but cannot get it as the lock is already acquired by the second thread.
