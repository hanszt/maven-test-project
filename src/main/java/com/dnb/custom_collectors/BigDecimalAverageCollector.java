...
class BigDecimalAverageCollector implements
        Collector<BigDecimal, BigDecimalAverageCollector.BigDecimalAccumulator, BigDecimal> {

    BigDecimalAverageCollector() {
    }
    ...
    static final class BigDecimalAccumulator {
        ...
        private BigDecimal getAverage() {
            final var SCALE = 2;
            return BigDecimal.ZERO.compareTo(count) == 0 ? BigDecimal.ZERO : sum.divide(count, SCALE, RoundingMode.HALF_UP);
        }

        private BigDecimalAccumulator combine(BigDecimalAccumulator another) {
            return new BigDecimalAccumulator(sum.add(another.sum), count.add(another.count));
        }
