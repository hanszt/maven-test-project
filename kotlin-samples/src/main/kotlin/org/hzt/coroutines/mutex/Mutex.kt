package org.hzt.coroutines.mutex
...
class Mutex {
    /**
       ...
       uses AtomicInteger and ConcurrentLinkedQueue instead of embedding these data structures right here
    ...
    */
    ...
    suspend fun lock() {
        ...
        return suspendCoroutineUninterceptedOrReturn sc@ {
            ...
            val waiter = Waiter(it.intercepted())
            waiters.add(waiter)
    ...
    fun unlock() {
        while (true) { // lock-free loop on state
            ...
            if (curState == 0) {
                ...
                if (state.compareAndSet(curState, curState - 1)) {
                    (retrieveWaiter() ?: throw IllegalStateException("Must have a waiter!")).continuation.resume(Unit)
                    return
                }
            }
        }
    ...
    private fun retrieveWaiter(): Waiter? {
        while (true) {
            ...
            if (!waiter.resumed) return waiter
            // otherwise it is an artifact, just look for the next one
        }
    }

    private class Waiter(val continuation: Continuation<Unit>) {
        var resumed = false
    }
}
