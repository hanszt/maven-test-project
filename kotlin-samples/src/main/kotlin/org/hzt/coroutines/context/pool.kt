package org.hzt.coroutines.context

import org.hzt.coroutines.run.launch
...
open class Pool(private val pool: ForkJoinPool) : AbstractCoroutineContextElement(ContinuationInterceptor),
    ...
    fun runParallel(block: suspend () -> Unit) = pool.execute { launch(this, block) }
}
...
private class PoolContinuation<T>(
    ...
    override fun resumeWith(result: Result<T>) = pool.execute { cont.resumeWith(result) }
}
