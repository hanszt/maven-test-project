 ...
 * as described by Fredman and Tarjan.  Fibonacci's heaps are interesting
 ...
 * for single-source the shortest paths can be shown to run in O(m + n lg n) using
...
public final class FibonacciHeap<T> {
    /* In order for all the Fibonacci heap operations to complete in O(1),
     ...
     * @return A new FibonacciHeap containing all the elements of both
     * heaps.
     */
    ...
    public Entry<T> dequeueMin() {
        ...
        /* Next, clear the parent fields of all the min element's children,
     ...
     * merges the two lists together into one circularly-linked
    ...
    private static <T> Entry<T> mergeLists(Entry<T> one, Entry<T> two) {
        ...
        if (one == null && two == null) { // Both null, resulting list is null.
            ...
            /* Return a pointer to whichever smaller. */
